...
FirebaseAuth::FirebaseAuth(QObject *parent)
    : QObject{parent}, d_ptr{new FirebaseAuthPrivate} {
  ...
  connect(d_ptr->updateKeysTimer, &QTimer::timeout, this,
          &FirebaseAuth::getGoogleSecureTokens);

  getGoogleSecureTokens();
}

FirebaseAuth::~FirebaseAuth() { delete d_ptr; }
...
void FirebaseAuth::verifyIdToken(
    const std::string &token, QObject *context,
  ...
  if (cb) {
    ...
    if (context && error == u"pubkey-notfound") {
      connect(this, &FirebaseAuth::publicKeysUpdated, context,
              [this, token, cb] {
                QString error;
                QJsonObject decodedToken = verifyIdToken(token, error);
...
QJsonObject FirebaseAuth::verifyIdToken(const std::string &token,
                                        QString &error) {
  QJsonObject ret;
  ...
  try {
    ...
    for (auto &[key, value] : decoded.get_payload_json()) {
      if (value.is<std::string>()) {
        ret.insert(QString::fromStdString(key),
                   QString::fromStdString(value.get<std::string>()));
      } else if (value.is<int64_t>()) {
        ret.insert(QString::fromStdString(key),
                   static_cast<qint64>(value.get<int64_t>()));
      } else if (value.is<double>()) {
        ret.insert(QString::fromStdString(key), value.get<double>());
      } else {
        ret.insert(QString::fromStdString(key),
                   QString::fromStdString(value.to_str()));
        qCWarning(FIREBASE_AUTH)
            << "SAVING CLAIM as JSON" << key.c_str() << value.to_str().c_str();
        break;
      }
    }
    ...
    if (ret[u"iss"].toString() !=
        u"https://securetoken.google.com/" + projectId) {
    ...
    auto verifier =
        jwt::verify().allow_algorithm(jwt::algorithm::rs256{it.value()});
...
void FirebaseAuth::getGoogleSecureTokens() {
  QNetworkRequest req(
      QUrl(QStringLiteral("https://www.googleapis.com/robot/v1/metadata/x509/"
  ...
  connect(reply, &QNetworkReply::finished, this, [=, this] {
    ...
    if (reply->error()) {
      ...
      if (error.error) {
        qCWarning(FIREBASE_AUTH)
            << "Google PubKeys ERROR" << error.errorString();
      } else {
        ...
        qCDebug(FIREBASE_AUTH)
            << "Got Google PubKeys" << d_ptr->m_googlePubKeys.keys();
  ...
  connect(reply, &QNetworkReply::finished, this,
          &FirebaseAuth::publicKeysUpdated);
}
